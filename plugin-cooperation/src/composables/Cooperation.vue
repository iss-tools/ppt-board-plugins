<template>
  <div class="cooperation-plugin-overlay">
    ...
    <div v-if="isConnected" class="pointer-events-none absolute inset-0 overflow-hidden z-[100]">
      <div v-for="(user, id) in remoteUsers" :key="id" v-show="user.x !== undefined && user.y !== undefined"
        class="absolute transition-all duration-150 ease-out flex flex-col items-start"
        :style="{ transform: `translate(${(user.x || 0) * scale + offsetX}px, ${(user.y || 0) * scale + offsetY}px)` }">
    ...
    <button class="cooperation-toggle" :class="{ dark: isDark }"
      :title="isConnected ? t('cooperationConnected') : t('cooperationInvite')" @click="toggleWindow">
      <svg v-if="isConnected" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2"
    ...
    <div v-if="showWindow && !isConnected" class="cooperation-window" :class="themeClass">
      <n-button text class="absolute top-4 right-4 transition z-10"
      ...
      <div class="p-8 text-center pt-10">
        ...
        <p class="text-sm mb-4" :class="isDark ? 'text-gray-300' : 'text-gray-700'">{{ t('inviteDescription') }}</p>
        <p class="text-[13px] mb-8 px-2 leading-relaxed" :class="isDark ? 'text-gray-400' : 'text-gray-500'">
          {{ t('privacyNotice') }}
        </p>

        <n-button type="primary" color="#6965db" block size="large" @click="handleStartSession">
          <template #icon>
            <n-icon><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round">
    ...
    <n-modal v-model:show="showSessionModal" preset="card" class="w-[720px] max-w-[95vw]" :title="t('modalTitle')"
      :theme-overrides="isDark ? { color: '#1e1e1e', textColor: '#fff' } : undefined">
      <div class="flex flex-col md:flex-row gap-6 p-2">
        <!-- Left: Edit Link -->
        <div class="flex-1 space-y-4">
          <h3 class="font-bold text-lg m-0" :class="isDark ? 'text-gray-200' : 'text-gray-800'">{{ t('editSessionTitle')
            }}
          </h3>
          ...
          <div class="flex justify-center p-6 rounded-xl border"
            :class="isDark ? 'bg-[#2a2a2a] border-[#333]' : 'bg-[#f8f9fa] border-gray-100'">
            <n-qr-code :value="editUrl" :size="180" color="#6965db" :background="isDark ? '#2a2a2a' : '#f8f9fa'" />
          </div>

          <div>
            <label class="block text-xs font-bold mb-1" :class="isDark ? 'text-gray-400' : 'text-gray-500'">{{
              t('sessionLink') }}</label>
            <n-input-group>
              <n-input :value="editUrl" readonly size="large"
                :style="isDark ? 'background: #2a2a2a; color: #fff' : 'background: #f8f9fa'" :bordered="false" />
        ...
        <!-- Right: View Link -->
        <div class="flex-1 space-y-4">
          <h3 class="font-bold text-lg m-0" :class="isDark ? 'text-gray-200' : 'text-gray-800'">{{ t('viewSessionTitle')
            }}
          </h3>
          <p class="text-sm m-0 min-h-[40px]" :class="isDark ? 'text-gray-400' : 'text-gray-500'">{{
            t('viewSessionDesc') }}
          ...
          <div class="flex justify-center p-6 rounded-xl border"
            :class="isDark ? 'bg-[#2a2a2a] border-[#333]' : 'bg-[#f8f9fa] border-gray-100'">
            <n-qr-code :value="viewUrl" :size="180" color="#475569" :background="isDark ? '#2a2a2a' : '#f8f9fa'" />
          </div>
          ...
          <div>
            <label class="block text-xs font-bold mb-1" :class="isDark ? 'text-gray-400' : 'text-gray-500'">{{
              t('shareLink') }}</label>
            <n-input-group>
              <n-input :value="viewUrl" readonly size="large"
                :style="isDark ? 'background: #2a2a2a; color: #fff' : 'background: #f8f9fa'" :bordered="false" />
      ...
      <template #footer>
        <div class="flex justify-between items-center w-full pt-2">
          <div class="text-sm flex items-center gap-1" :class="isDark ? 'text-gray-400' : 'text-gray-500'">
            {{ t('encryptionNotice') }}
          </div>
          <n-button :color="isDark ? '#4a1d1d' : '#fef2f2'" :text-color="isDark ? '#fc8181' : '#ef4444'"
            class="border-red-200" @click="disconnectAndClose">
            {{ t('stopSession') }}
          </n-button>
        </div>
      </template>
...
import { NButton, NIcon, NDivider, NInput, NInputGroup, NModal, NQrCode, createDiscreteApi } from 'naive-ui';
...
const localKey = ref(Math.random().toString(36).substring(2, 10) + Math.random().toString(36).substring(2, 10));
...
function disconnectAndClose() {
  ...
  localKey.value = Math.random().toString(36).substring(2, 10) + Math.random().toString(36).substring(2, 10);
  showSessionModal.value = false;
}
